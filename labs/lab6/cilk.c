...
void encode(char* plainText, char* cypherText, xorKey* keyList, int ptextlen, int numKeys) {
  ...
  for(charLoop=0;charLoop<ptextlen;charLoop++) {
    CILK_C_REDUCER_OPXOR(cipherChar, char, plainText[charLoop]); 
    CILK_C_REGISTER_REDUCER(cipherChar);
    cilk_for(keyLoop=0;keyLoop<numKeys;keyLoop++) {
       REDUCER_VIEW(cipherChar) = REDUCER_VIEW(cipherChar) ^ getBit(&(keyList[keyLoop]),charLoop);
    }
    cypherText[charLoop]= REDUCER_VIEW(cipherChar);  
    CILK_C_UNREGISTER_REDUCER(cipherChar);
