...
void apply_prewitt(pixel * const in)
{
	double prewittKernelX[3*3];
	double prewittKernelY[3*3];
			...
			for(int x = i - radius, kx = 0; x <= i + radius; ++x, ++kx) {
				for(int y = j - radius, ky = 0; y <= j + radius; ++y, ++ky) {
					if(x >= 0 && x < rows && y >= 0 && y < cols) {
						const int in_offset = x + (y*rows);
						const int k_offset = kx + (ky*dim);
						prewittX[out_offset].red   += prewittKernelX[k_offset] * in[in_offset].red;
						prewittX[out_offset].green += prewittKernelX[k_offset] * in[in_offset].green;
						prewittX[out_offset].blue  += prewittKernelX[k_offset] * in[in_offset].blue;
						
						prewittY[out_offset].red   += prewittKernelY[k_offset] * in[in_offset].red;
						prewittY[out_offset].green += prewittKernelY[k_offset] * in[in_offset].green;
						prewittY[out_offset].blue  += prewittKernelY[k_offset] * in[in_offset].blue;
					}
				}
			}
		}
